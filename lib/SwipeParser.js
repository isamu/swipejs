...
function () {
  ...
  _createClass(SwipeParser, null, [{
    ...
    key: "transformedPath",
    value: function transformedPath() {// not implement
    }
  }, {
    key: "parseTransform",
    value: function parseTransform() {// not implement
    }
  }, {
    key: "is",
    ...
    value: function inheritProperties(obj, tempObj) {
      ...
      if (tempObj) {
        ...
        for (var keyString in tempClone) {
          var tempValue = tempClone[keyString]; // $.each(tempClone, (keyString, tempValue) => {
          ...
          if (ret[keyString] == null) {
            ...
            if (SwipeParser.is("Array", ret[keyString]) && SwipeParser.is("Array", ret_val)) {
              idMap = {}; // $.each(ret_val, (index, tempItem) => {
    ...
    value: function localizedStringForKey(key) {
      ...
      if (strings = pageinfo["strings"]) {
        var text = strings[key]; // todo localize
    ...
    value: function parseFontName(value, markdown) {
      ...
      return "sans-serif, Helvetica"; // return [];
    }
  }, {
    key: "parseShadow",
